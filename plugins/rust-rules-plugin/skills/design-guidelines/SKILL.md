...
description: Rust設計ガイドライン。振る舞いの配置（implと自由関数の使い分け）、肥大化したファイルの分割、型変換（From/TryFrom）の実装位置と依存方向、enumの命名、重複排除の手段選択を定義。設計判断・リファクタリング時に参照。
---

# Rust設計ガイドライン

コードの構造・配置に関する設計判断の指針を定義するスキル。
